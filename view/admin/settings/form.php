        ...
        ?>
            <div class="mb-3">
                ...
                <?php elseif ($fieldType === 'file'): ?>
                    <input type="file" name="favicon_file" accept=".png,.ico,.svg,image/png,image/x-icon,image/vnd.microsoft.icon,image/svg+xml">
                    <?php if ($fieldValue !== ''): ?>
                        <div class="mt-2">
                            <div class="text-muted"><?= htmlspecialchars($fieldValue, ENT_QUOTES, 'UTF-8') ?></div>
                            <img src="<?= htmlspecialchars($url($fieldValue), ENT_QUOTES, 'UTF-8') ?>" alt="favicon preview" style="width:32px;height:32px">
                        </div>
                    <?php endif; ?>
                <?php else: ?>
