...
$rowRenderer = static function (array $row) use ($url, $formatDateTime, $icon, $t, $isEditor, $currentUserId, $thumbSuffix): string {
    ...
    <tr>
        ...
        <td class="table-col-actions">
            <?php if ($canManage): ?>
                <button class="btn btn-light btn-icon" type="button" data-media-delete-open="<?= $id ?>" data-modal-open data-modal-target="#media-delete-modal" aria-label="<?= htmlspecialchars($t('media.delete', 'Delete media'), ENT_QUOTES, 'UTF-8') ?>" title="<?= htmlspecialchars($t('media.delete', 'Delete media'), ENT_QUOTES, 'UTF-8') ?>">
                    <?= $icon('delete') ?>
