...
<form
    ...
    action="<?= $escUrl($mode === 'add' ? $url('admin/api/v1/media/add') : $url('admin/api/v1/media/' . (int)($item['id'] ?? 0) . '/edit')) ?>"
    data-api-submit
>
    <?= $csrfField() ?>
    <div class="content-editor-layout">
        <div class="card p-4">
            ...
            <?php if ($mode === 'add'): ?>
                <div class="mb-3">
                    <label><?= $escHtml($t('media.file')) ?></label>
                    <div class="custom-upload-field" data-custom-upload-auto-submit>
                        ...
                        <input id="media-file-upload" type="file" name="file" accept="<?= $escHtml((string)($imageUploadAccept ?? '')) ?>" required>
                    </div>
                    <small class="text-muted d-block mt-2"><?= $escHtml(sprintf($t('common.allowed_upload_types'), (string)($imageUploadTypesLabel ?? ''))) ?></small>
                    <?php if (!empty($errors['file'])): ?><small class="text-danger"><?= $escHtml((string)$errors['file']) ?></small><?php endif; ?>
                </div>
            <?php endif; ?>
            ...
            <?php if ($mode === 'edit'): ?>
                <?php if ($previewUrl !== ''): ?>
                    <div class="content-thumbnail-preview mb-3">
                        <img src="<?= $escUrl($previewUrl) ?>" alt="<?= $escHtml((string)($item['name'] ?? '')) ?>">
                    </div>
                <?php endif; ?>
                <h3 class="mb-3"><?= $escHtml($t('media.used_in')) ?></h3>
                <?php if (($usages ?? []) === []): ?>
                    <p class="text-muted m-0"><?= $escHtml($t('media.no_usage')) ?></p>
                <?php else: ?>
                    <div class="table-responsive">
                        <table class="table">
                            <thead>
                            <tr>
                                <th><?= $escHtml($t('content.post')) ?></th><th><?= $escHtml($t('common.created')) ?></th><th><?= $escHtml($t('media.usage_origin')) ?></th>
                            </tr>
                            </thead>
                            <tbody>
        ...
        <?php if ($mode === 'edit'): ?>
            <aside class="content-editor-sidebar">
                <div class="card">
                    <div class="content-box-header"><?= $escHtml($t('common.actions')) ?></div>
                    <div class="p-3">
                        <div class="mb-3">
                            <label><?= $escHtml($t('common.created')) ?></label>
                            <div class="text-muted"><?= $escHtml($formatDateTime((string)($item['created'] ?? ''))) ?></div>
                        </div>
                        <div class="m-0">
                ...
                <?php if ($fileMeta !== null): ?>
                    <div class="card">
                        <div class="content-box-header"><?= $escHtml($t('media.file')) ?></div>
                        <div class="p-3">
                            <div class="mb-3">
                                <label><?= $escHtml($t('media.path')) ?></label>
                                <div class="text-muted"><?= $escHtml((string)($item['path'] ?? '')) ?></div>
                            </div>
                            <div class="mb-3">
                                <label><?= $escHtml($t('media.filename')) ?></label>
                                <div class="text-muted"><?= $escHtml((string)$fileMeta['filename']) ?></div>
                            </div>
                            <div class="mb-3">
                                <label><?= $escHtml($t('media.mime')) ?></label>
                                <div class="text-muted"><?= $escHtml((string)$fileMeta['mime']) ?></div>
                            </div>
                            <div class="mb-3">
                                <label><?= $escHtml($t('media.extension')) ?></label>
                                <div class="text-muted"><?= $escHtml((string)$fileMeta['extension']) ?></div>
                            </div>
                            <div class="mb-3">
                                <label><?= $escHtml($t('media.dimensions')) ?></label>
                                <div class="text-muted"><?= $escHtml((string)$fileMeta['dimensions']) ?></div>
                            </div>
                            <div class="m-0">
                                <label><?= $escHtml($t('media.size')) ?></label>
                                <div class="text-muted"><?= $escHtml((string)$fileMeta['size']) ?></div>
                            </div>
                        </div>
                    </div>
                <?php endif; ?>
...
<?php if ($mode === 'edit'): ?>
    <form
        ...
        method="post"
        action="<?= $escUrl($url('admin/api/v1/media/' . (int)($item['id'] ?? 0) . '/delete')) ?>"
        data-api-submit
    >
        <?= $csrfField() ?>
    </form>
    <div class="modal-overlay" data-modal id="media-delete-modal">
        <div class="modal">
            <p data-modal-text><?= $escHtml($t('media.delete_confirm')) ?></p>
            <div class="modal-actions">
                <button class="btn btn-light" type="button" data-modal-close><?= $escHtml($t('common.cancel')) ?></button>
                <button class="btn btn-primary" type="button" data-modal-confirm data-form-id="media-delete-form"><?= $escHtml($t('common.confirm')) ?></button>
            </div>
        </div>
    </div>
<?php endif; ?>
