    ...
    <div class="row justify-center">
        <div class="col-12 col-md-6">
            <div class="card p-5">
                ...
                <h1 class="m-0 mb-4"><?= esc_html(t('auth.login')) ?></h1>
                <p class="mb-3 text-danger" data-api-form-message hidden></p>
                <form method="post" action="<?= esc_url($url('admin/api/v1/auth/login')) ?>" data-api-submit>
                    <?= $csrfField() ?>
                    <div class="mb-3">
                    ...
                    <div class="mb-4">
                        <label><input type="checkbox" name="remember" value="1" <?= ((int)($old['remember'] ?? 0) === 1) ? 'checked' : '' ?>> <?= esc_html(t('auth.remember')) ?></label>
                    </div>
                    <button class="btn btn-primary" type="submit"><?= esc_html(t('auth.login')) ?></button>
                </form>
                <?php if (($allowRegistration ?? false) === true): ?>
                    <p class="mt-3 mb-0">
                        <a href="<?= esc_url($url('auth/register')) ?>"><?= esc_html(t('auth.create_account')) ?></a>
                    </p>
                <?php endif; ?>
                <p class="mt-2 mb-0">
                    <a href="<?= esc_url($url('auth/lost')) ?>"><?= esc_html(t('auth.lost_password')) ?></a>
                </p>
            </div>
        </div>
    </div>
