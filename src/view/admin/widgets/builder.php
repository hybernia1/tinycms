...
$inactiveAreaItems = is_array($inactiveAreaItems ?? null) ? $inactiveAreaItems : [];
$defaultArea = (string)($areas[0] ?? '');
$builderAreaScreensPrefix = trim((string)($builderAreaScreensPrefix ?? ''));
...
$inactiveItemsByArea = [];
if ($builderAreaScreensPrefix === '') {
    foreach ($inactiveAreaItems as $item) {
        $area = trim((string)($item['area'] ?? ''));
        if ($area !== '') {
            $inactiveItemsByArea[$area][] = (array)$item;
        }
    }
}
...
$renderFields = static function (array $definition, array $data, string $index): void {
    foreach ((array)($definition['fields'] ?? []) as $field):
        ...
        ?>
        <div class="builder-field form-floating-field">
...
$renderRow = static function (array $item, string $index, bool $unused = false) use ($widgets, $areas, $areaLabels, $renderFields): void {
            ...
            <div class="builder-actions">
                <?php if ($unused && $areas !== []): ?>
                    <div class="widget-unused-move" data-widget-unused-controls>
                        <select data-widget-unused-target aria-label="<?= esc_attr(t('widgets.move_to_area')) ?>">
                            <?php foreach ($areas as $targetArea): ?>
                                <option value="<?= esc_attr($targetArea) ?>"><?= esc_html((string)($areaLabels[$targetArea] ?? $targetArea)) ?></option>
                            <?php endforeach; ?>
                        </select>
                        <button class="btn btn-light btn-icon builder-action" type="button" data-widget-unused-move aria-label="<?= esc_attr(t('widgets.move_unused')) ?>" title="<?= esc_attr(t('widgets.move_unused')) ?>">
                            <?= icon('restore') ?>
                        </button>
                    </div>
                <?php endif; ?>
                <button class="btn btn-light btn-icon builder-action builder-action-danger" type="button" data-widget-item-remove aria-label="<?= esc_attr(t('widgets.remove_item')) ?>" title="<?= esc_attr(t('widgets.remove_item')) ?>">
                    <?= icon('delete') ?>
                </button>
            </div>
        </div>
        <div class="builder-details" data-widget-item-details hidden>
...
<div class="builder-areas">
    <?php if ($inactiveItemsByArea !== []): ?>
        <section class="card p-4 widget-unused-card" data-widget-unused-card>
            <div class="widget-unused-head">
                <span class="widget-unused-icon"><?= icon('warning') ?></span>
                <div>
                    <h2 class="m-0"><?= esc_html(t('widgets.unused_title')) ?></h2>
                    <p class="text-muted m-0"><?= esc_html(t('widgets.unused_description')) ?></p>
                </div>
            </div>
            <div class="widget-unused-areas">
                <?php foreach ($inactiveItemsByArea as $area => $areaItems): ?>
                    <div class="widget-unused-area" data-widget-unused-area="<?= esc_attr($area) ?>">
                        <input type="hidden" name="managed_area[]" value="<?= esc_attr($area) ?>">
                        <div class="widget-unused-area-head">
                            <strong><?= esc_html(sprintf(t('widgets.unused_area'), $area)) ?></strong>
                            <span class="badge" data-widget-unused-count><?= count($areaItems) ?></span>
                        </div>
    ...
    <?php foreach ($areas as $area): ?>
        ...
        <?php endif; ?>
        <div class="card p-4 builder-card" data-widget-area="<?= esc_attr($area) ?>">
            ...
            <?php if ($widgets !== []): ?>
                <div class="builder-add" data-widget-add-area="<?= esc_attr($area) ?>">
                    <button class="btn btn-light btn-icon builder-add-toggle" type="button" data-widget-add-toggle aria-expanded="false" aria-label="<?= esc_attr(t('widgets.add_item')) ?>" title="<?= esc_attr(t('widgets.add_item')) ?>">
                        <?= icon('add') ?>
                    </button>
                    <div class="builder-add-form" data-widget-add-form hidden>
                        <div class="builder-field form-floating-field">
        ...
        <?php endif; ?>
    <?php endforeach; ?>
    <?php if ($builderAreaScreensPrefix === '' && $areas === []): ?>
        <div class="card p-4">
            <p class="text-muted m-0"><?= esc_html(t('widgets.no_areas')) ?></p>
        </div>
    <?php elseif ($builderAreaScreensPrefix === '' && $widgets === []): ?>
        <div class="card p-4">
